...
import { Communion } from "@/models/communion";
...
export async function GET(
  ...
  const communion = new Communion(calendar);
  const communionData = communion.getAll();

  if (isAll) {
    return NextResponse.json({ liturgicalYear, ...communionData });
  }

  const dateString = calendar.getToday().format("YYYY-MM-DD");
  return NextResponse.json({
    liturgicalYear,
    [dateString]: communionData[dateString] || [],
  });
}
